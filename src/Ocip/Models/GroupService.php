...
class GroupService extends \MyCLabs\Enum\Enum
{
    ...
    const ACCOUNT_AUTHORIZATION_CODES = 'Account/Authorization Codes';

    const AUTO_ATTENDANT = 'Auto Attendant';

    const AUTO_ATTENDANT_VIDEO = 'Auto Attendant - Video';

    const AUTO_ATTENDANT_STANDARD = 'Auto Attendant - Standard';

    const CALL_CAPACITY_MANAGEMENT = 'Call Capacity Management';

    const CALL_PARK = 'Call Park';

    const CALL_PICKUP = 'Call Pickup';

    const CITY_WIDE_CENTREX = 'City-Wide Centrex';

    const CUSTOM_RINGBACK_GROUP = 'Custom Ringback Group';

    const CUSTOM_RINGBACK_GROUP_VIDEO = 'Custom Ringback Group - Video';

    const EMERGENCY_ZONES = 'Emergency Zones';

    const ENHANCED_OUTGOING_CALLING_PLAN = 'Enhanced Outgoing Calling Plan';
    ...
    const LDAP_INTEGRATION = 'LDAP Integration';

    const MEET_ME_CONFERENCING = 'Meet-Me Conferencing';

    const MUSIC_ON_HOLD = 'Music On Hold';

    const MUSIC_ON_HOLD_VIDEO = 'Music On Hold - Video';

    const OUTGOING_CALLING_PLAN = 'Outgoing Calling Plan';

    const PREFERRED_CARRIER_GROUP = 'Preferred Carrier Group';

    const ROUTE_POINT = 'Route Point';

    const SERIES_COMPLETION = 'Series Completion';

    const SERVICE_SCRIPTS_GROUP = 'Service Scripts Group';

    const TRUNK_GROUP = 'Trunk Group';

    const VOICE_MESSAGING_GROUP = 'Voice Messaging Group';
